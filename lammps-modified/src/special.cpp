...
void Special::build()
{
  ...
  if (me == 0 && screen) {
    ...
    fprintf(screen,"Finding 1-2 1-3 1-4 neighbors ...\n"
                   "  special bond factors lj:   %-10g %-10g %-10g %-10g\n"
                   "  special bond factors coul: %-10g %-10g %-10g %-10g\n",
                   special_lj[1],special_lj[2],special_lj[3], special_lj[4],
                   special_coul[1],special_coul[2],special_coul[3], special_coul[4]);
  }
  ...
  // -----------------------------------------------------
  // done if special_bond weights for 1-3, 1-4, 1-5 are set to 1.0
  // -----------------------------------------------------

  if (force->special_lj[2] == 1.0 && force->special_coul[2] == 1.0 &&
      force->special_lj[3] == 1.0 && force->special_coul[3] == 1.0 &&
      force->special_lj[4] == 1.0 && force->special_coul[4] == 1.0) {
    dedup();
    combine();
    fix_alteration();
    return;
  ...
  // done if special_bond weights for 1-4, 1-5 are set to 1.0
  ...
  if(force->special_lj[4] == 1.0 && force->special_coul[4] == 1.0) {
    dedup();
    if (force->special_angle) angle_trim();
  ...
  // -----------------------------------------------------
  // compute nspecial[i][3] = # of 1-5 neighbors of atom i
  // -----------------------------------------------------

  // nbufmax = largest buffer needed to hold info from any proc
  // info for each atom = 2 scalars + list of 1-4 neighbors

  nbuf = 0;
  for (i = 0; i < nlocal; i++) nbuf += 2 + nspecial[i][2];
  memory->create(buf,nbuf,"special:buf");

  // fill buffer with:
  // (1) = counter for 1-5 neighbors, initialized to 0
  // (2) = # of 1-4 neighbors
  // (3:N) = list of 1-4 neighbors

  size = 0;
  for (i = 0; i < nlocal; i++) {
    buf[size++] = 0;
    buf[size++] = nspecial[i][2];
    for (j = 0; j < nspecial[i][2]; j++) buf[size++] = onefour[i][j];
  }

  // cycle buffer around ring of procs back to self
  // when receive buffer, scan list of 1-4 neighbors for atoms I own
  // when find one, increment 1-5 count by # of 1-2 neighbors of my atom
  //   may include duplicates and original atom but they will be culled later

  sptr = this;
  comm->ring(size,sizeof(tagint),buf,10,ring_nspecial_onefive,buf);
  ...
  max = 0;
  for (i = 0; i < nlocal; i++) max = MAX(max,nspecial[i][3]);
  MPI_Allreduce(&max,&maxall,1,MPI_INT,MPI_MAX,world);

  if (me == 0) {
    if (screen) fprintf(screen,"  %d = max # of 1-5 neighbors\n",maxall);
    if (logfile) fprintf(logfile,"  %d = max # of 1-5 neighbors\n",maxall);
  }

  memory->create(onefive,nlocal,maxall,"special:onefive");

  // nbufmax = largest buffer needed to hold info from any proc
  // info for each atom = 3 scalars + list of 1-4 neighs + list of 1-5 neighs

  nbuf = 0;
  for (i = 0; i < nlocal; i++)
    nbuf += 3 + nspecial[i][2] + nspecial[i][3];
  memory->create(buf,nbuf,"special:buf");
  ...
  sptr = this;
  comm->ring(size,sizeof(tagint),buf,6,ring_onefive_lists,buf);
  ...
  j = 0;
  for (i = 0; i < nlocal; i++) {
    if (buf[j+2] != nspecial[i][3]) {
      fprintf(screen, "1-4 neighs of %i: ", i);
      for( k = 0; k < nspecial[i][2]; k++) {
        fprintf(screen, " %i", atom->map(onefour[i][k]));
      }
      fprintf(screen, "\n");
      for( k = 0; k < nspecial[i][2]; k++ ) {
        fprintf(screen, "1-2 neighs of %i: ", atom->map(onefour[i][k]));
        int m = atom->map(onefour[i][k]);
        for( int l = 0; l < nspecial[m][0]; l++) {
          fprintf(screen, "%i ", atom->map(onetwo[m][l]));
        }
        fprintf(screen, "\n");
      }
      error->one(FLERR, "1-5 bond count is inconsistent");
    }
    j += 3 + nspecial[i][2];
    for (k = 0; k < nspecial[i][3]; k++)
      onefive[i][k] = buf[j++];
  }
...
/* ----------------------------------------------------------------------
   remove duplicates within each of onetwo, onethree, onefour, onefive individually
------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------
   ...
   convert nspecial[0], nspecial[1], nspecial[2], nspecial[3] into cumulative counters
------------------------------------------------------------------------- */
...
void Special::combine()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (j = 0; j < nspecial[i][3]; j++) atom->map_one(onefive[i][j],-1);
  }
  ...
  // ----------------------------------------------------
  // fill special array with 1-2, 1-3, 1-4, 1-5 neighs for each atom
  // ----------------------------------------------------

  // again use map to cull duplicates
  // exclude original atom explicitly
  // adjust nspecial[i] values to reflect removed duplicates
  // nspecial[i][1] and nspecial[i][2], nspecial[i][3]  now become cumulative counters
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    atom->map_one(tag[i],-1);
    for (j = 0; j < nspecial[i][3]; j++) atom->map_one(special[i][j],-1);
  }
...
void Special::ring_nspecial_onefive(int ndatum, char *cbuf)
{
  Atom *atom = sptr->atom;
  int **nspecial = atom->nspecial;
  int nlocal = atom->nlocal;
  ...
  if(i != ndatum) sptr->error->one(FLERR, "Invalid buffer format");
}

/* ----------------------------------------------------------------------
   when receive buffer, scan list of 1-4 neighbors for atoms I own
   when find one, add its neighbors to 1-5 list
     incrementing the count in buf(i+4)
     this process may include duplicates but they will be culled later
------------------------------------------------------------------------- */

void Special::ring_onefive_lists(int ndatum, char *cbuf)
{
  Atom *atom = sptr->atom;
  int **nspecial = atom->nspecial;
  int nlocal = atom->nlocal;

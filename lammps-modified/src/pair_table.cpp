...
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable,tmp;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jtype = type[j];
      if (itype > jtype) {
          lambda = prefactor[jtype][itype];}
      else {
          lambda = prefactor[itype][jtype];}
	
      if (lambda != 1.0) {
          l2 = lambda * lambda * lambda;
          oneminuslambda = 1.0 - lambda;
          oneminuslambda *= oneminuslambda; 
          rsq += oneminuslambda;
          rsq *= (cutsq[itype][jtype] - 1.0 + lambda) / cutsq[itype][jtype];}
      else l2 = 1.0;

     if (rsq < cutsq[itype][jtype]) {
        tb = get_table(i,j,itype,jtype);
        if (rsq < tb->innersq) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          fpair = l2 * factor_lj * tb->f[itable];
...
void PairTable::allocate()
{
  ...
  memory->create(prefactor,nt,nt,"pair:prefactor");

  memset(&setflag[0][0],0,nt*nt*sizeof(int));
  memset(&cutsq[0][0],0,nt*nt*sizeof(double));
  memset(&tabindex[0][0],0,nt*nt*sizeof(int));
  memset(&prefactor[0][0],0,nt*nt*sizeof(double));
}
...
{
  if (narg != 6 && narg != 6) error->all(FLERR,"Illegal pair_coeff command");
  ...
  bcast_table(tb);
  // set table cutoff
  int ilop,ihip,jlop,jhip;
  force->bounds(FLERR,arg[0],atom->ntypes,ilop,ihip);
  force->bounds(FLERR,arg[1],atom->ntypes,jlop,jhip);

  if (narg == 6) tb->cut = force->numeric(FLERR,arg[4]);
  else if (tb->rflag) tb->cut = tb->rhi;
  else tb->cut = tb->rfile[tb->ninput-1];
  double prefactor_one = force->numeric(FLERR,arg[4]);
  if (narg == 6) prefactor_one = force->numeric(FLERR,arg[5]);
  int countp = 0;
  for (int i = ilop; i <= ihip; i++) {
    for (int j = MAX(jlop,i); j <= jhip; j++) {
      prefactor[i][j] = prefactor_one;
      prefactor[j][i] = prefactor_one;
      // setflag[i][j] = 1; think about it
      countp++;
    }
  }
  ...
  //Change by SASHA: next line was like this: if (rlo <= 0.0) error->all(FLERR,"Invalid pair table cutoff");
  ...
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      if( !setflag[i][j] ) tabindex[i][j] = ntables;
      setflag[i][j]++;
      count++;
    }
  }
...
std::map<std::string, long> PairTable::fill_seek_cache(FILE* fp) {
  std::map<std::string, long> seekpos;
  char line[MAXLINE];
  Table * tb = (Table*)memory->smalloc(sizeof(Table), "temp_pair");
...
//                    &itmp,&rfile,&tb->efile[i],&tb->ffile[i]))  ++cerror;
    ...
    {
      char * tline = line;
      itmp = (int)strtol(tline, &tline, 10);
      rfile = strtod(tline, &tline);
                         ...
                         double &fforce)
{
  ...
  Table *tb = get_table(i, j, itype, jtype);
  rsq *= (cutsq[itype][jtype] - 1.0 + lambda) / cutsq[itype][jtype];
  ...
  if (tabstyle == LOOKUP) {
    ...
    fforce = l2 * factor_lj * tb->f[itable];
  } else if (tabstyle == LINEAR) {
    itable = static_cast<int> ((rsq-tb->innersq) * tb->invdelta);
    if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
    fraction = (rsq - tb->rsq[itable]) * tb->invdelta;
    value = tb->f[itable] + fraction*tb->df[itable];
    fforce = l2 * factor_lj * value;
  } else if (tabstyle == SPLINE) {
    itable = static_cast<int> ((rsq-tb->innersq) * tb->invdelta);
    if (itable >= tlm1) error->one(FLERR,"Pair distance > table outer cutoff");
    b = (rsq - tb->rsq[itable]) * tb->invdelta;
  ...
  return l2 * factor_lj*phi;
}

/*------------------------------------------------------------------------*/

PairTable::Table * PairTable::get_table(int i, int j, int itype, int jtype) {
  //Silence unused parameters warning
  (void)i,(void)j;
  return &tables[tabindex[itype][jtype]];
}
...
{
  if (strcmp(str,"prefactor") == 0){
    dim = 2;
    return (void *) prefactor;
  }
  if ((strcmp(str,"cut_coul") != 0) && (strcmp(str,"prefactor") != 0)) return NULL;

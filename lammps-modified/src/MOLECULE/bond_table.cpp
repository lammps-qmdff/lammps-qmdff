...
std::map<std::string, long> BondTable::fill_seek_cache(FILE* fp) {
...
//                    &itmp,&tb->rfile[i],&tb->efile[i],&tb->ffile[i])) ++cerror;
    ...
    {
      char * tline = line;
      itmp = (int)strtol(tline, &tline, 10); e = errno;
      tb->rfile[i] = strtod(tline, &tline); e = errno;
      tb->efile[i] = strtod(tline, &tline); e = errno;
      tb->ffile[i] = strtod(tline, &tline); e = errno;
    }
...
void BondTable::uf_lookup(int type, double x, double &u, double &f, int i, int j)
{
  int itable;
  double fraction,a,b;
  char estr[128];
...
BondTable::Table *BondTable::get_table(int type, int i, int j) {
  (void)i, (void)j;

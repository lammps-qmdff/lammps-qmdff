...
class AngleTable : public Angle {
 ...
 protected:
  ...
  virtual Table* get_table(int type, int i, int j, int k);

  void param_extract(Table *, char *);
  void spline(double *, double *, int, double, double, double *);
  double splint(double *, double *, double *, int, double);

  void uf_lookup(int, double, double &, double &, int, int, int);
  void u_lookup(int, double, double &, int, int, int);

  std::map<std::string, std::map<std::string, long>> seek_pos_cache;
  virtual std::map<std::string, long> fill_seek_cache(FILE* fp);
};

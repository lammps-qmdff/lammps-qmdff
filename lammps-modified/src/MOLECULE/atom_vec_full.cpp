...
void AtomVecFull::grow(int n)
{
  ...
  nspecial = memory->grow(atom->nspecial,nmax,4,"atom:nspecial");
...
void AtomVecFull::copy(int i, int j, int delflag)
{
  ...
  nspecial[j][3] = nspecial[i][3];
  for (k = 0; k < nspecial[j][3]; k++) special[j][k] = special[i][k];
...
int AtomVecFull::pack_exchange(int i, double *buf)
{
  ...
  buf[m++] = ubuf(nspecial[i][3]).d;
  for (k = 0; k < nspecial[i][3]; k++) buf[m++] = ubuf(special[i][k]).d;
...
int AtomVecFull::unpack_exchange(double *buf)
{
  ...
  nspecial[nlocal][3] = (int) ubuf(buf[m++]).i;
  for (k = 0; k < nspecial[nlocal][3]; k++)
...
int AtomVecFull::unpack_restart(double *buf)
{
  ...
  nspecial[nlocal][0] = nspecial[nlocal][1] = nspecial[nlocal][2] = nspecial[nlocal][3] = 0;
...
int AtomVecFull::pack_data_one(int i, double **buf)
{
  int m = 0;
...
bigint AtomVecFull::memory_usage()
{
  ...
  if (atom->memcheck("nspecial")) bytes += memory->usage(nspecial,nmax,4);

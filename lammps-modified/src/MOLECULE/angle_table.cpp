...
void AngleTable::coeff(int narg, char **arg)
{
  ...
  for (int i = ilo; i <= ihi; i++) {
    if( !setflag[i] ) tabindex[i] = ntables;
    setflag[i]++;
    theta0[i] = tb->theta0;
    count++;
  }
...
double AngleTable::single(int type, int i1, int i2, int i3)
{
  ...
  double theta = acos(c);
  double u=0.0;
  u_lookup(type,theta,u, i1, i2, i3);
  return u;
}
...
std::map<std::string, long> AngleTable::fill_seek_cache(FILE* fp) {
...
void AngleTable::uf_lookup(int type, double x, double &u, double &f, int i, int j, int k)
{
  int itable;
  double fraction,a,b;

  Table *tb = get_table(type, i, j, k);
...
void AngleTable::u_lookup(int type, double x, double &u, int i, int j, int k)
{
  int itable;
  double fraction,a,b;
...
AngleTable::Table *AngleTable::get_table(int type, int i, int j, int k) {
  (void)i, (void)j, (void)k;
  return  &tables[tabindex[type]];
}
...
class NPair : protected Pointers {
 ...
 protected:
  ...
  inline int find_special(const tagint *list, const int *nspecial,
    ...
    const int n1 = nspecial[0]; //1-2
    const int n2 = nspecial[1]; //1-3
    const int n3 = nspecial[2]; //1-4
    const int n4 = nspecial[3]; //1-5
    ...
    for (int i = 0; i < n4; i++) {
      if (list[i] == tag) {
        if (i < n1) {
          ...
          if (special_flag[4] == 0) return -1;
          else if (special_flag[4] == 1) return 0;
          else return 4;
        }
      }
    }
